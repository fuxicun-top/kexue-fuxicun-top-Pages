const Version = '2026-07-21 07:00:00';
let config_JSON, 缓存SOCKS5白名单 = null, 调试日志打印 = false;
let SOCKS5白名单 = ['*tapecontent.net', '*cloudatacdn.com', '*loadshare.org', '*cdn-centaurus.com', 'scholar.google.com'];
const Pages静态页面 = 'https://fuxicun-top.github.io/kexue-fuxicun-top-Pages';
///////////////////////////////////////////////////////全局常量和工具函数///////////////////////////////////////////////
...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			// 仅代理首页相关静态资源到 GitHub Pages（login/admin/sub 等由 Worker 内部处理）
			if (!访问路径 || 访问路径.startsWith('asset/') ||
				访问路径 === 'favicon.ico' || 访问路径 === 'robots.txt') {
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				try {
					...
					const 订阅TOKEN = await MD5MD5(host + userID), 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('kexue-fuxicun-top-Pages');
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							try {
								const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + 'kexue-fuxicun-top-Pages (https://github.com/fuxicun-top/kexue-fuxicun-top-Pages)' } });
								if (response.ok) {
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	...
	const cidr_url = 运营商文件标识 === 'cf' ? `https://raw.githubusercontent.com/fuxicun-top/kexue-fuxicun-top-Pages/main/CF-CIDR.txt` : `https://raw.githubusercontent.com/fuxicun-top/kexue-fuxicun-top-Pages/main/CF-CIDR/${运营商文件标识}.txt`;
...
async function 获取优选订阅生成器数据(优选订阅生成器HOST) {
	...
	try {
		const response = await fetch(优选订阅生成器URL, {
			headers: { 'User-Agent': 'v2rayN/kexue-fuxicun-top-Pages (https://github.com/fuxicun-top/kexue-fuxicun-top-Pages)' }
		});
